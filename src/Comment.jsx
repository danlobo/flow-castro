...
const Comment = ({ 
    ...
    const mouseDownHandler = useCallback((e) => {
        ...
        const mouseMoveHandler = (e) => {
            ...
            onMove?.({ x: position.x + dx / screenScale, y: position.y + dy / screenScale })
        }
        const mouseUpHandler = (e) => {
            const dx = e.pageX - startX;
            const dy = e.pageY - startY;

            onMoveEnd?.({ x: position.x + dx / screenScale, y: position.y + dy / screenScale })
    ...
    onContextMenu={onContextMenu}>
        ...
        <input className={css.title} placeholder="Comment" value={title ?? ''} onChange={(e) => onChangeTitle(e.target.value)} onMouseDown={(e) => e.stopPropagation()} />
        <textarea className={css.value} value={text ?? ''} onChange={(e) => onChangeText(e.target.value)} onMouseDown={(e) => e.stopPropagation()}/>
    </div>;
}
